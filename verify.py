...
def verify_random_num_gen_bl(lines: list[str]):
    calls_bit_rotate_l = False
    for line in lines:
        toks = line.split()
        if toks[0].strip().upper().startswith("BL"):
            if toks[1].strip().upper() == "BIT_ROTATE_L":
                calls_bit_rotate_l = True
    if not calls_bit_rotate_l:
        return False, "\n\tfunction fails to call BL bit_rotate_l"
    return True, ""
...
with open(sys.argv[1], "r") as infile:
    ...
    while not infile.readline().strip().startswith("random_num_gen"):
        pass
    lines = []
    for line in infile:
        ...
        if len(line.strip()) == 0:
            continue
        lines.append(line.strip())
    result, reason = verify_random_num_gen_bl(lines)
    if not result:
        print(f"random_num_gen failed verification. Reason: {reason}")
